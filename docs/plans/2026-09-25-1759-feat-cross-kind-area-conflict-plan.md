...
- KTD10. **Exactly one side of a collision is undone, chosen deterministically.** For a mining-versus-farming collision the mining side is the offender and the farm is never unassigned — that follows from R3 and R4, which already say the farm is entitled to the ground. Same-dock pairs never reach this rule at all (R7a). For a same-kind collision across two docks neither requirement settles it, so the area with the higher `(owning dock id, area id)` pair is the offender; any total order would do, and the point is that the outcome is identical on every load. `AllPublishedAreas()` walks `IWorldObjectManager.All`, whose order is not stable, so without a rule the same save can undo dock A on one load and dock B on the next.
  **The rule is applied greedily, against what is still standing.** Asking it pairwise against the world as it arrived is complete but not minimal: in a chain where A and B share ground and B and C share other ground while A and C never touch, undoing B settles both, yet C is also the offender against B when asked in isolation. A pairwise pass therefore takes C's assignment for a rival the same pass removed, and records a reason naming an area that is now unassigned. The walk visits areas in the order the rule favours and asks each one only about areas not already undone, which settles a chain of any length in one pass and undoes nothing whose ground is no longer contested. Covers R13, R16.

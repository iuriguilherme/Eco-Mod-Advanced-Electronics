...
- R2. Mining job status stays per mining dock: a job belongs to the dock running it, not to the area. Two docks do not work one area, because R37's claim holds it against every other dock and R39 refuses the second assignment — this requirement predates the claim and once described that configuration as supported. What survives is where job state lives, which is what the split between shared ground truth and per-dock work still rests on.
...
- R30. An area carries a kind — the job it exists for — as a property of the area itself, not of the dock that owns it. Kind reaches the player through the exclusive status slot, the same one `[surveyed]`, `[mined]`, `[cleared]` and `[empty]` occupy: a farming area reads `[farm]` there. It is never a field of its own and never an annotation competing for an overlay slot. An area shows the status that belongs to what it is for, so the mining lifecycle is what a mining area reads and `[farm]` is what a farming area reads. That is how a player knows what an area is for and what to expect when they try to assign a drone to it. A dock may list areas of any kind. R34 already makes areas objects two docks compare across the world; an object compared that way has to be able to say what it is for.
- R46. A mining status and `[farm]` never appear together, and the mechanism that prevents it is structural rather than a precedence rule. The area's kind selects which status is derived at all: a mining area runs the lifecycle ladder, a farming area reads `[farm]`, and the ladder is never consulted for farmland. Deriving a mining status and then overwriting it leaves a path where both values exist and only render order separates them. This matters from the moment kind lands on the area under R30: ground repurposed from an exhausted mine keeps its mined stamps and bedrock observations, so every input the ladder reads is still present and still true about its past.
...
- R39. An assignment that would claim plots another area already holds is refused, and the refusal names which plots and what holds them. Kind plays no part: any assigned area holds its plots against every dock, and any unassigned area is free to a dock of any kind. The single exception is `[empty]` — ground with nothing left to take is assignable even where another area still covers it, which is what lets land pass from one purpose to the next. Claiming is atomic against concurrent assignment: two docks assigning overlapping ground at the same moment cannot both succeed, and the one that loses is told why rather than silently working ground it does not hold.
  ...
  Because the rule is kind-blind, it also orders the two jobs: a mining dock is refused an area its survey dock still holds, so a mining drone waits for the survey to finish rather than working the ground alongside it. Each strategy retires its own assignment when its work is done, so the refusal stands only while a drone is actually on that ground. It is a refusal and not a queue on purpose — the scheduler that orders work when docks share a drone is out of this plan's scope, and saying "not yet" needs no scheduler.
...
- Farming drone — one drone running every farming job, planned in `docs/plans/2026-08-22-0010-feat-farming-drones-plan.md`.
  - Depends on this plan: `[farm]` is a status in the exclusive slot R3 defines, not an annotation — a farming area reads `[farm]` where a mining area reads its lifecycle. `[flat]` is an uncoloured annotation of the kind R9 defines, with the lowest display priority of any of them: it shows only when nothing else needs the room, and yields to `[farm]` and to every trip fact. Its redraw behaviour relies on R20, and R43 is what keeps a farming write from being mislabelled or silently unsurveying ground.
...
2. Kind selects the status slot's value, per R30 and R46: a farming area reads `[farm]` there and the mining ladder is never run for it. Repurposing an exhausted mining area keeps its stamps and observations, so the guard must be that the ladder is not consulted, not that its result is overwritten — the same overlay vocabulary `[assigned]` and `[overlap]` already use. A dock may list areas of any kind, and the tag is what tells the player what each one is for.

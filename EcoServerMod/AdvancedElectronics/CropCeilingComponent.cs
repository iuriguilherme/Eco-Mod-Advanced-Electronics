...
namespace Eco.Mods.TechTree
{
    ...
    public class CropCeilingComponent : WorldObjectComponent
    {
        ...
        /// <summary>
        /// Upper bound of a row's stepper. An editable number on a mod tab needs a declared
        /// range or every edit snaps back to its old value
        /// (docs/solutions/runtime-errors/autogen-template-binding-contract.md). Large enough
        /// that no storage a dock can link to would hold more of one crop.
        /// </summary>
        private const int MaxCeiling = 1000000;
        ...
        public void RefreshAll()
        {
            ...
            if (!this.overflowResolved)
            {
                ...
                var overflow = CropCatalog.All.Where(c => !RowKeys.Contains(c.Key)).Select(c => c.UniqueName).ToList();
                if (overflow.Count > 0)
                {
        ...
        private void SetFromRow(string key, int value)
        {
            ...
            if (dock.CropCeilingFor(key) == value) return;
